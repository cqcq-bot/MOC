  ...
  return (
    <div className="hero__wall" aria-hidden="true">
      <div className="hero__wall-plane">
        {wallSets.map((wallSet, columnIndex) => (
          <div className="hero__wall-column" key={columnIndex}>
            <div className="hero__wall-track">
              {[0, 1].map((copyIndex) => (
                <div className="hero__wall-copy" aria-hidden={copyIndex === 1} key={copyIndex}>
                  {wallSet.map((image) => (
                    <div className="hero__wall-tile" key={`${copyIndex}-${image}`}>
                      <img src={asset(image)} alt="" draggable={false} loading={copyIndex ? "lazy" : undefined} />
                    </div>
                  ))}
                </div>
              ))}
